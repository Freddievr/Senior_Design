...
def get_measurement_keithley_2401():
    ...
    global num_fingers
    global gap_width
    num_fingers_int = int(num_fingers)
    strip_width = 2             # in mm              
    numReadings = str(num_fingers_int*2)
    keithley.timeout = 11000 * num_fingers_int             # timeout set up according to # of fingers
    ...
    row_array = []
    distance_array = []
    for i in range(0, num_fingers_int, 1) :
        row_array.append("Finger_" + str(i+1))
        distance_array.append(float(gap_width)*(i+1))
    ...
    # Setup Temp DATAFRAME FOR CALCULATIONS     # CALCULATE INTERCEPTS
...
if __name__ == "__main__":
  ...
  # pyinstaller finalgui.py --onefile --windowed 