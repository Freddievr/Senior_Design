...
def get_measurement_keithley_2401():
    ...
    df2 = df.drop(columns = ['Current-','Current+'])
    print(df2)
    ax = df2.plot(figsize=(num_fingers+1,7))
    plt.xticks(fontsize=10)
    ax.set_xlabel('SI Test Sample', fontsize= 10)
    ax.set_ylabel('Voltage', fontsize = 10)
    ax.set_title("Voltage vs Finger #", fontsize = 20)
    plt.show()
